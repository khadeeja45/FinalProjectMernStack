...
router.delete("/delte_employee/:id", (req, res) => {
    User.findByIdAndRemove(req.params.id, (err, foundUser) => {
        if (err) res.json({ err: true, messaage: " Failed to delete employee" });
        res.json({ err: false, message: "delte employee suceessfully" });
    });
});

router.put("/edit_employees/:id", (req, res) => {
    User.findByIdAndUpdate(req.params.id, req.body.employee, (err, updatedEmployee) => {
        if (err) res.json({ err: true, message: "update unsuccessfull" });
        res.json({ err: false, message: "successfully edited the user" });
    });
});
